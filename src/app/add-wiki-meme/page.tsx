...
export default function AddWikiMeme() {
    ...
    const handleSubmit = async (e: { preventDefault: () => void; }) => {
        ...
        formData.append('memeName', sendData.memeName);
        formData.append('description', sendData.description);
        formData.append('memeImage', sendData.memeImage ?? "");
        formData.append('memeExample', sendData.memeExample ?? "");
        try {
            const response = await fetch('/api/add-wiki-meme', {
                method: 'POST',
                body: formData,
            });
    ...
    return (
        <>
            ...
            <main className="flex min-h-screen flex-col items-center justify-between p-24 bg-gray-100 dark:bg-gray-900">
                <h1 className="mt-10 mb-4 text-4xl font-bold text-gray-900 dark:text-gray-100">Add Wiki Meme</h1>
                <form onSubmit={handleSubmit} className="max-w-lg mx-auto p-6 bg-white dark:bg-gray-800 shadow-md rounded-md space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Meme Name</label>
                        <input
                            type="text"
                            name="memeName"
                            value={sendData.memeName}
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Example Meme</label>
                        <input
                            type="file"
                            name="memeExample"
                            accept="image/jpeg"
